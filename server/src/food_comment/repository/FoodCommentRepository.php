...
namespace src\food_comment\repository;
 ...
 */
class FoodCommentRepository implements Repository
{

    /**
     */
    public static function listFoodComment($FoodID): array
    {
        $query = "SELECT * FROM comment WHERE FoodID=$FoodID;";
        $result = null;
        try {
        ...
        if ($result) {
            while ($row = $result->fetch_array(MYSQLI_ASSOC)) {
                ...
                $row["ImageList"] = self::getFoodCommentImage($row["CommentID"]);
                $row["Reply"] = self::getReplyComment($row["CommentID"]);

                error_log(json_encode($row), 0);
                array_push($list_food_comment, $row);
            }
        }

        error_log("FOOD_COMMENT_REPOSITORY::FETCH_LIST::", 0);
        return $list_food_comment;
    }

    public static function findFoodCommentByID($CommnetID)
    {
        $query = "SELECT * FROM comment WHERE CommentID = $CommnetID AND FoodID!=0;";
    ...
    public static function getReplyComment($CommentID)
    {
        ...
        error_log("FOOD_COMMENT_REPOSITORY::FETCH_REPLY::", 0);
        return $list_comment_reply;
    }

    public static function getFoodCommentImage($CommentID)
    {
    ...
    public static function create($entity = null): \mysqli_result|bool|null
    {
        ...
        $comment_query = "INSERT INTO comment (UserID, Content, NewsID, FoodID) VALUES('$UserID','$entity->Content', 0, '$entity->FoodID');";
        try {
