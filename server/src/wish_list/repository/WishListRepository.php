...
class WishListRepository implements Repository
{
    ...
    public static function listWishList(): array
    {
        ...
        while ($row = $result->fetch_array(MYSQLI_ASSOC)) {
            ...
            // TODO: Fetch food / combo data
            // * Add the fetched one into result
             if($row["FoodID"] != 0) {
                 $food_data = FoodRepository::findFoodByID($row["FoodID"]);
                 $row["data"] = $food_data;
             }
             else if($row["ComboID"] != 0) {
                 $combo_data = ComboRepository::findComboByID($row["ComboID"]);
                 $row["data"] = $combo_data;
             }
